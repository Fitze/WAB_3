...
\color{black}
	...
	\chapter{Technologien und Verfahren}
		...
		\section{Informationsverschlüsselung}
			...
			Es gibt mittlerweile verschiedene Möglichkeiten \ac{E2EE} umzusetzen. Im Grunde müssen durch die Nutzer jedoch Einstellungen in den E-Mailprogrammen wie Thunderbird, Outlook oder Mail.app vorgenommen werden oder zusätzliche Software installiert werden. Demzufolge besteht ein erhöhter Einrichtungsaufwand für die Nutzer zur Sicherstellung der Vertraulichkeit und um in Sicherheitsniveaus der Stufe 3 oder 4 (Vgl. \prettyref{tab:sicherheitsniveaus}) zu kommunizieren.\medskip
			...
			\subsection{S/MIME - Secure / Multipurpose Internet Mail Extensions}
				...
				%Problem
				Die bislang betrachteten Verfahren bieten keine Möglichkeit einer \ac{E2EE}. \ac{PGP} hat als erstes Protokoll diese Lücke gefüllt. Mit \ac{SMIME} kam 4 Jahre später ein zweiter Standard hinzu.\footcite{Duevel}
				\medskip\\
							
				%Idee
				\ac{SMIME} ist eine Erweiterung des MIME Datentyps, der für die Signierung und/oder Verschlüsselung von Nachrichten entwickelt worden ist.
				Bislang konnten E-Mails nur im ASCII-Zeichensatz versendet werden. Mit dem Anbruch des digitalen Zeitaltern stieg jedoch die Forderung auch andere Datentypen austauschen zu können. \ac{SMIME} setzt dieses Bedürfnis um.\footcite[][S. 57-60]{Schwenk}
				\medskip
				
				%Voraussetzung
				Damit ein Nutzer \ac{SMIME} verwenden kann, benötigt er ein X.509 Zertifikat. Dieses wird in seiner einfachsten Form kostenlos von einer \ac{CA} ausgestellt.\footcite{Duevel}
				...
				%Umsetzung
				Hat sich der Absender für die Verschlüsselung via \ac{SMIME} entschieden, sucht der Mail Client automatisch in der Zertifikatsdatenbank nach einem Zertifikat, welches die E-Mail Adresse des Empfängers enthält. Sollte ein solches Zertifikat nicht gefunden werden, wird der Nutzer darüber informiert, dass eine \ac{SMIME} Verschlüsselung nicht möglich sei. Ist ein entsprechendes Zertifikat vorhanden, wird die E-Mail hybrid verschlüsselt.\footcite[][S. 65]{Schwenk}
...
%				- Anschließend base64 codiert, damit Mailserver den Inhalt nicht unbeabsichtigt ändern kann
				...
				Um die Lesbarkeit von \ac{SMIME} signierten E-Mails auch für Nutzer zu gewährleisten, die dieses Verfahren nicht verwenden, stehen zwei verschiedene Datentypen zur Verfügung. Auf eine genauere Beschreibung soll an dieser Stelle verzichtet und stattdessen auf den Schwenk\footcite[][S. 65]{Schwenk} verwiesen werden.
				\medskip
...
%				\footcite[][S. 65]{Schwenk}
				...
				%Vorteile
				Der Vorteil von \ac{SMIME} liegt darin, "dass Verschlüsselung und Signatur genauso einfach zu bedienen sind, wie z.B. [..] das Anfügen eines Attachments"\footcite[][S. 61]{Schwenk}. Außerdem wird \ac{SMIME} durch viele Browser by Default unterstützt.\footcite{Duevel}
				Durch die zwei verschiedenen Datentypen, können \ac{SMIME} signierte Nachrichten auch von E-Mail Clients interpretiert werden, die dieses Verfahren nicht verwenden. 
				\newpage
...
%			Daher wird dieser Schlüssel auch als Secure Entry Point bezeichnet.
	...
	\chapter{E-Mail-Sicherheit in der Praxis}
		\section{Provider Vergleich}
			...
			Für die eingesetzten Zertifikate erhalten alle Provider wiederum volle Punktzahl.\footcite{vgl. Anhang SSL Reports}
			\medskip
			
			Zwar haben seit den Enthüllungen von Edward Snowden zu den Machenschaften der NSA und anderen Geheimdiensten die meisten E-Mail Provider die Initiative ergriffen und  auf sicherere Verfahren umgestellt\footcite{vgl. Anhang SSL Reports}. Ein voll umfänglichen Schutz durch eine \ac{E2EE} wird bislang jedoch noch nicht geboten. Einzig und alleine Google hat bereits für seinen E-Mail Dienst ein AddOn in einer Alpha Version entwickelt, mit Hilfe dessen eine PGP Verschlüsselung für den Webmailer ermöglicht werde.\footcite{Kirsch} Eine Beschreibung für die Einrichtung einer \ac{E2EE} unter Verwendung eines Mail Clients war nicht aufzufinden.
			...
			\end{quote}  
			Circa ein dreiviertel Jahr später wird ein weiterer Vorfall veröffentlicht, in welchem Administratoren sich direktes Zugang zu den Mailinhalten der Nutzer verschafft hätten.\footcite{Mailbox2014Microsoft}
			\medskip
			...
			Yahoomail kann zusätzlich mit einer Wegwerfadresse, einem Anmeldesiegel (vgl. Abbildung \ref{fig:Yahoo_Anmeldesiegel}) sowie dem selbst entwickelten \ac{DKIM} Verfahren überzeugen. \ac{DKIM} basiert auf asymmetrischer Kommunikation und stellt die Authentizität von E-Mail Absendern sicher.\footcite{DKIM}.
...
%			\footnote{Vgl. ebd.}
		...
		\subsubsection{Vertraulichkeit und Integrität}		
			...
			Die Ergebnisse des SSL-Test (\prettyref{sec:nwa}) sind für die drei Provider ähnlich gut. GMX und WEB unterstützen Forward Secrecy mit den meisten Browsern, T-Online hingegen lässt den Schlüsselaustausch für ältere Browser über RC4 zu.
		...
		\subsubsection{Sicherheitsrelevante Aspekte}
			Für die Mails auf den EmiG-Servern der EmiG-Provider besteht keine separate verschlüsselte Speicherung der E-Mails, insofern dies durch den Nutzer nicht selbst geschehen ist. (\prettyref{subsec:pgp})
...
%			Zertifikate müssen für Teilnehmer der Provider von deutschen \acp{CA} ausgestellt sein.\medskip\\	
	...
	\chapter*{Anhang}
		...
		\label{LastPage}
%		\includepdf[pages=1-4]{anlagen/SSL_Server_Test-yahoo.pdf}
%		\includepdf[pages=1-4]{anlagen/SSL_Server_Test-gmail.pdf}
%		\includepdf[pages=1-5]{anlagen/SSL_Server_Test-hotmail.pdf}
%		\includepdf[pages=1-4]{anlagen/SSL_Server_Test-t-online.pdf}
%		\includepdf[pages=1-4]{anlagen/SSL_Server_Test-web.pdf}
%		\includepdf[pages=1-4]{anlagen/SSL_Server_Test-gmx.pdf}
%		\includepdf[pages=1-4]{anlagen/SSL_Server_Test-mailbox.pdf}
%		\includepdf[pages=1-4]{anlagen/SSL_Server_Test-posteo.pdf}
%		\includepdf[pages=1-2]{anlagen/DNSSEC_Analyzer-yahoo.pdf}
%		\includepdf[pages=1]{anlagen/DNSSEC_Analyzer-gmail.pdf}
%		\includepdf[pages=1]{anlagen/DNSSEC_Analyzer-hotmail.pdf}
%		\includepdf[pages=1]{anlagen/DNSSEC_Analyzer-t-online.pdf}
%		\includepdf[pages=1]{anlagen/DNSSEC_Analyzer-web.pdf}
%		\includepdf[pages=1]{anlagen/DNSSEC_Analyzer-gmx.pdf}
%		\includepdf[pages=1]{anlagen/DNSSEC_Analyzer-mailbox.pdf}
%		\includepdf[pages=1]{anlagen/DNSSEC_Analyzer-posteo.pdf}
\end{document}